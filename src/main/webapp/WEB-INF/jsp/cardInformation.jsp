...
<table align="center" width="80%">
    ...
    <c:forEach items="${requestScope.cards}" var="card" >
        <tr>
            ...
            <td>
                <form  action="${pageContext.request.contextPath}/cardInformation" method="post">
                    <input type="hidden" name="hidden" value="${card.cardId}">
                    <input onclick="setTimeout(function () { window.location.reload(); }, 3)" type="submit"
                           name="button1" value=<fmt:message key="table.BlockCard"/>/>
                </form>
            </td>
            <td>
                <form action="${pageContext.request.contextPath}/cardInformation" method="post">
                    <input type="hidden" name="hidden" value="${card.cardId}">
                <input onclick="setTimeout(function () { window.location.reload(); }, 3)" type="submit"
                       name="button2" value=${card.unblockRequest}>
                </form>
            </td>
        </tr>
    </c:forEach>
...
<%--    <td><a href="user-cards?page=${currentPage - 1}">Previous</a></td>--%>
<%--</c:if>--%>
...
<%--                </c:when>--%>
<%--                <c:otherwise>--%>
<%--                    <td><a href="user-cards?page=${i}">${i}</a></td>--%>
<%--                </c:otherwise>--%>
<%--            </c:choose>--%>
<%--        </c:forEach>--%>
<%--    </tr>--%>
...
<%--    <td><a href="user-cards?page=${currentPage + 1}">Next</a></td>--%>
<%--</c:if>--%>
